...
import { useUpdateWithdrawal } from "@/api/wallet/withdrawal/hook";
import { Withdrawal } from "@/types/front/wallet/withdrawal";

interface Props {
  data: Withdrawal;
}
export default function EditWithdrawalDialog({ data }: Props) {
  ...
  const [receiverAddress, setReceiverAddress] = useState(data.receiverAddress);
